...
fn main() {
    ...
    let mut computer = Computer::new(&memory, Some(&input));
    computer.run();
    eprintln!("{:?}", computer.get_state());
    eprintln!("{:?}", computer.get_output());
}
